/*
   ...
   ----------------------------
   This example is to verify the operation of the USB module. This example runs
   in interrupt mode. USB interrupts are configured and ISR is registered using
   Interrupt module. After initializing and configuring the USB module, when there 
   is any request from the USB host application USB ISR is triggered and the 
   requested operation is performed inside the ISR.
...
void setup()
{
    ...
    usbConfig.opMode             = USB_OPMODE_POLLED;
